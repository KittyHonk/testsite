...
import {checkFertilizerSpring, createFertilizerSpring, getAllFertilizerSpring} from "../../http/TableApi";
...
const TableBody = React.forwardRef((props, ref) => {
    ...
    useEffect(() => {
        checkFertilizerSpring(row_owner, date.current).then(data => {
            getAllFertilizerSpring(row_owner, date.current).then(data => {
                setValue(data)
            })
        })
    }, [date.current])
    ...
    useImperativeHandle(ref, () => ({
        newRow (date) {
            createFertilizerSpring(
                ...
                getAllFertilizerSpring(row_owner, date).then(data => {
                    setValue(data)
                })
            })
        },
    ...
    if (value.length !== 0){
        return (
            <tr>
                ...
                <td><InputField user={user.role} start={value[0].value6} ref={refList.ref5}>{value[0].value6}</InputField></td>
                <td><InputField start={value[0].value7} ref={refList.ref6}>{value[0].value7}</InputField></td>
                <td>{value[0].result67}</td>
        ...
        return (
            <tr>
                ...
                <td><InputField user={user.role} ref={refList.ref5}></InputField></td>
