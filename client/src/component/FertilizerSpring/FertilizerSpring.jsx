...
import {getAllFertilizerSpring} from "../../http/TableApi";
...
const FertilizerSpring = observer((props) => {
    ...
    const getValue = async () => {
        ...
        for (let i = 0; i < props.rowName.length; i++) {
            if ((props.rowName[i].name === user.region) || (user.role === "ADMIN")) {
                await getAllFertilizerSpring(props.rowName[i].name, date.current).then(data => {
                    valueList.push(...data)
                })
            }
        }
    ...
    const calcField = (valueList) => {
        let sumList = new Array(14).fill(0)
        for (let i = 0; i < valueList.length; i++) {
            if (valueList[i] !== undefined) {
                ...
                sumList[9] = ((sumList[8]/sumList[7])*100).toFixed(2)
        ...
        <div classTable="divMain">
            ...
            >
                ...
                <tr>
                    ...
                    <th>План завоза, тонн</th>
                    <th colSpan={13}>Накопленно тонн</th>
                </tr>
                <tr>
                    <th></th>
                    <th></th>
                    <th>Всего</th>
                    <th>%</th>
                    <th>План ам. селитра</th>
                    <th>В т.ч. ам. селитра</th>
                    <th>%</th>
                    <th>Их них произв. ОАО Минудобрения</th>
                    <th>План азофоски</th>
                    <th>В т.ч. азофоски</th>
                    <th>%</th>
                    <th>Из них произв. ОАО Минудобрения</th>
                    <th>В т.ч. безв. аммика вода</th>
                    <th>В т.ч. КАС</th>
                    <th>В т.ч. другие виды</th>
                </tr>
                </thead>
            ...
            <div className="bottomBar">
                ...
                <Export fileName={`Удобрения весна ${date.current}`} tableRef={tableRef}/>
            </div>
        </div>
    );
});

export default FertilizerSpring;