...
import {checkSunflowerHarvest, createSunflowerHarvest, getAllSunflowerHarvest} from "../../http/TableApi";
...
const TableBody = React.forwardRef((props, ref) => {
    ...
    useEffect(() => {
        checkSunflowerHarvest(row_owner, date.current).then(data => {
            getAllSunflowerHarvest(row_owner, date.current).then(data => {
                setValue(data)
            })
        })
    }, [date.current])
    ...
    useImperativeHandle(ref, () => ({
        newRow (date) {
            createSunflowerHarvest(
                ...
                getAllSunflowerHarvest(row_owner, date).then(data => {
                    setValue(data)
                })
            })
        },
