...
import {checkSoyHarvest, createSoyHarvest, getAllSoyHarvest} from "../../http/TableApi";
...
const TableBody = React.forwardRef((props, ref) => {
    ...
    useEffect(() => {
        checkSoyHarvest(row_owner, date.current).then(data => {
            getAllSoyHarvest(row_owner, date.current).then(data => {
                setValue(data)
            })
        })
    }, [date.current])
    ...
    useImperativeHandle(ref, () => ({
        newRow (date) {
            createSoyHarvest(
                ...
                getAllSoyHarvest(row_owner, date).then(data => {
                    setValue(data)
                })
            })
        },
    ...
    if (value.length !== 0){
        return (
            <tr>
                ...
                <td>{value[0].result23}</td>
                <td><InputField start={value[0].value3} ref={refList.ref2}>{value[0].value3}</InputField></td>
                <td><InputField user={user.role} start={value[0].value4} ref={refList.ref3}>{value[0].value4}</InputField></td>
                ...
                <td><InputField user={user.role} start={value[0].value7} ref={refList.ref6}>{value[0].value7}</InputField></td>
                <td><InputField start={value[0].value8} ref={refList.ref7}>{value[0].value8}</InputField></td>
                <td>{value[0].result89}</td>
                <td><InputField start={value[0].value9} ref={refList.ref8}>{value[0].value9}</InputField></td>
                <td><InputField user={user.role} start={value[0].value10} ref={refList.ref9}>{value[0].value10}</InputField></td>
                ...
                <td><InputField user={user.role} start={value[0].value13} ref={refList.ref12}>{value[0].value13}</InputField></td>
                <td><InputField start={value[0].value14} ref={refList.ref13}>{value[0].value14}</InputField></td>
                <td>{value[0].result1415}</td>
                <td><InputField start={value[0].value15} ref={refList.ref14}>{value[0].value15}</InputField></td>
                <td><InputField user={user.role} start={value[0].value16} ref={refList.ref15}>{value[0].value16}</InputField></td>
                ...
                <td><InputField user={user.role} start={value[0].value19} ref={refList.ref18}>{value[0].value19}</InputField></td>
                <td><InputField start={value[0].value20} ref={refList.ref19}>{value[0].value20}</InputField></td>
                <td>{value[0].result2021}</td>
