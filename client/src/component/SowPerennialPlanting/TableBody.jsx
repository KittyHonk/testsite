...
import {checkSowPerennialPlanting, createSowPerennialPlanting, getAllSowPerennialPlanting} from "../../http/TableApi";
...
const TableBody = React.forwardRef((props, ref) => {
    ...
    useEffect(() => {
        checkSowPerennialPlanting(row_owner, date.current).then(data => {
            getAllSowPerennialPlanting(row_owner, date.current).then(data => {
                setValue(data)
            })
        })
    }, [date.current])
    ...
    useImperativeHandle(ref, () => ({
        newRow (date) {
            createSowPerennialPlanting(
                ...
                getAllSowPerennialPlanting(row_owner, date).then(data => {
                    setValue(data)
                })
            })
        },
