...
import {checkTopDressing, createTopDressing, getAllTopDressing} from "../../http/TableApi";
...
const TableBody = React.forwardRef((props, ref) => {
    ...
    const refList = {
        ...
        ref5: useRef(), ref6: useRef(), ref7: useRef(),
    } 

    useEffect(() => {
        checkTopDressing(row_owner, date.current).then(data => {
            getAllTopDressing(row_owner, date.current).then(data => {
                setValue(data)
            })
        })
    }, [date.current])
    ...
    useImperativeHandle(ref, () => ({
        newRow (date) {
            createTopDressing(
                ...
                getAllTopDressing(row_owner, date).then(data => {
                    setValue(data)
                })
            })
        },
