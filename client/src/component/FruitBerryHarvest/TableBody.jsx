...
import {checkFruitBerryHarvest, createFruitBerryHarvest, getAllFruitBerryHarvest} from "../../http/TableApi";
...
const TableBody = React.forwardRef((props, ref) => {
    ...
    const refList = {
        ...
        ref20: useRef(), ref21: useRef(), ref22: useRef(), ref23: useRef(),
    } 

    useEffect(() => {
        checkFruitBerryHarvest(row_owner, date.current).then(data => {
            getAllFruitBerryHarvest(row_owner, date.current).then(data => {
                setValue(data)
            })
        })
    }, [date.current])
    ...
    useImperativeHandle(ref, () => ({
        newRow (date) {
            createFruitBerryHarvest(
                ...
                getAllFruitBerryHarvest(row_owner, date).then(data => {
                    setValue(data)
                })
            })
        },
