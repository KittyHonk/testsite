...
import {getAllBeetHarvest} from "../../http/TableApi";
...
const BeetHarvest = observer((props) => {
    ...
    const getValue = async () => {
        ...
        for (let i = 0; i < props.rowName.length; i++) {
            if ((props.rowName[i].name === user.region) || (user.role === "ADMIN")) {
                await getAllBeetHarvest(props.rowName[i].name, date.current).then(data => {
                    valueList.push(...data)
                })
            }
        }
    ...
    const calcField = (valueList) => {
        ...
        for (let i = 0; i < valueList.length; i++) {
            if (valueList[i] !== undefined) {
                ...
                sumList[8] = ((sumList[4]*10000)/(sumList[1]*1000)).toFixed(2)
                sumList[9] = ((sumList[5]*10000)/(sumList[1]*1000)).toFixed(2)
                sumList[10] += valueList[i].value8
                sumList[11] += valueList[i].value9
            }
        }
        setResult(sumList)
    }
        ...
        <div classTable="divMain">
            ...
            >
                ...
                <tr>
                    ...
                    <th>Площадь к уборке (план), тыс га</th>
                    <th>Убрано, тыс га</th>
                    <th>%</th>
                    <th>Убрано за день, га</th>
                    <th>Урожайность в пер. весе, ц/га</th>
                    <th>Урожайность в зачет. весе, ц/га</th>
                    <th>Валовый сбор в пер. весе, тыс. тонн</th>
                    <th>Валовый сбор в зачет. весе, тыс. тонн</th>
                    <th colSpan={2}>Вывезено свеклы</th>
                    <th colSpan={2}>Работало комбайнов</th>
                </tr>
                <tr>
                    <th></th>
                    <th></th>
                    ...
                    <th></th>
                    <th></th>
                    <th>На заводы, тыс. тонн</th>
                    <th>За пред. области, тыс. тонн</th>
                    <th>Всего шт</th>
                    <th>В т.ч. иностранных</th>
                </tr>
                </thead>
            ...
            <div className="bottomBar">
                ...
                <Export fileName={`Уборка свеклы ${date.current}`} tableRef={tableRef}/>
            </div>
        </div>
    );
});

export default BeetHarvest;