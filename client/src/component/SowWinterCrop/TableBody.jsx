...
import {checkSowWinterCrop, createSowWinterCrop, getAllSowWinterCrop} from "../../http/TableApi";
...
const TableBody = React.forwardRef((props, ref) => {
    ...
    useEffect(() => {
        checkSowWinterCrop(row_owner, date.current).then(data => {
            getAllSowWinterCrop(row_owner, date.current).then(data => {
                setValue(data)
            })
        })
    }, [date.current])
    ...
    useImperativeHandle(ref, () => ({
        newRow (date) {
            createSowWinterCrop(
                ...
                getAllSowWinterCrop(row_owner, date).then(data => {
                    setValue(data)
                })
            })
        },
