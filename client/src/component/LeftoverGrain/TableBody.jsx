...
import {checkLeftoverGrain, createLeftoverGrain, getAllLeftoverGrain} from "../../http/TableApi";
...
const TableBody = React.forwardRef((props, ref) => {
    ...
    const refList = {
        ...
        ref30: useRef(), ref31: useRef(), ref32: useRef(), ref33: useRef(),
    } 

    useEffect(() => {
        checkLeftoverGrain(row_owner, date.current).then(data => {
            getAllLeftoverGrain(row_owner, date.current).then(data => {
                setValue(data)
            })
        })
    }, [date.current])
    ...
    useImperativeHandle(ref, () => ({
        newRow (date) {
            createLeftoverGrain(
                ...
                getAllLeftoverGrain(row_owner, date).then(data => {
                    setValue(data)
                })
            })
        },
