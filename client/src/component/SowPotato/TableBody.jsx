...
import {checkSowPotato, createSowPotato, getAllSowPotato} from "../../http/TableApi";
...
const TableBody = React.forwardRef((props, ref) => {
    ...
    useEffect(() => {
        checkSowPotato(row_owner, date.current).then(data => {
            getAllSowPotato(row_owner, date.current).then(data => {
                setValue(data)
            })
        })
    }, [date.current])
    ...
    useImperativeHandle(ref, () => ({
        newRow (date) {
            createSowPotato(
                ...
                getAllSowPotato(row_owner, date).then(data => {
                    setValue(data)
                })
            })
        },
