...
import {checkSowVegetableAndGourd, createSowVegetableAndGourd, getAllSowVegetableAndGourd} from "../../http/TableApi";
...
const TableBody = React.forwardRef((props, ref) => {
    ...
    const refList = {
        ...
        ref10: useRef(), ref11: useRef(), ref12: useRef(),
    } 

    useEffect(() => {
        checkSowVegetableAndGourd(row_owner, date.current).then(data => {
            getAllSowVegetableAndGourd(row_owner, date.current).then(data => {
                setValue(data)
            })
        })
    }, [date.current])
    ...
    useImperativeHandle(ref, () => ({
        newRow (date) {
            createSowVegetableAndGourd(
                ...
                getAllSowVegetableAndGourd(row_owner, date).then(data => {
                    setValue(data)
                })
            })
        },
