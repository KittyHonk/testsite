...
import {getAllFertilizerAutumn} from "../../http/TableApi";
...
const FertilizerAutumn = observer((props) => {
    ...
    const getValue = async () => {
        ...
        for (let i = 0; i < props.rowName.length; i++) {
            if ((props.rowName[i].name === user.region) || (user.role === "ADMIN")) {
                await getAllFertilizerAutumn(props.rowName[i].name, date.current).then(data => {
                    valueList.push(...data)
                })
            }
        }
        ...
        <div classTable="divMain">
            ...
            >
                ...
                <tr>
                    ...
                    <th>План</th>
                    <th colSpan={8}>Накопленно тонн</th>
                    <th colSpan={3}>Из них производство ОАО Минудобрения</th>
                </tr>
                <tr>
                    <th></th>
                    <th></th>
            ...
            <div className="bottomBar">
                ...
                <Export fileName={`Удобрения осень ${date.current}`} tableRef={tableRef}/>
            </div>
        </div>
    );
});

export default FertilizerAutumn;