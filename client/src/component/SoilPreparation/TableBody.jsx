...
import {checkSoilPreparation, createSoilPreparation, getAllSoilPreparation} from "../../http/TableApi";
...
const TableBody = React.forwardRef((props, ref) => {
    ...
    useEffect(() => {
        checkSoilPreparation(row_owner, date.current).then(data => {
            getAllSoilPreparation(row_owner, date.current).then(data => {
                setValue(data)
            })
        })
    }, [date.current])
    ...
    useImperativeHandle(ref, () => ({
        newRow (date) {
            createSoilPreparation(
                ...
                getAllSoilPreparation(row_owner, date).then(data => {
                    setValue(data)
                })
            })
        },
