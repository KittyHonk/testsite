const {TableHarrowing} = require('../models/models')
...
class tableController {
    async getAll(req, res, next) {
        try {
            ...
            let table = await TableHarrowing.findAll(
                {
    ...
    async check(req, res) {
        ...
        let checkTable = await TableHarrowing.findAll({where: {row_owner, date}});
        if (checkTable.length == 0) {
            var name = await TableHarrowing.create({row_owner, date})
        }
    ...
    async create(req, res, next) {
        try {
            ...
            var name = await TableHarrowing.update({
                row_owner: row_owner,
                value1: value1,
                value2: value2,
                result12: result12,
