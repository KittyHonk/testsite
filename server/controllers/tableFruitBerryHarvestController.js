const {TableFruitBerryHarvest} = require('../models/models')
...
class tableController {
    async getAll(req, res, next) {
        try {
            ...
            let table = await TableFruitBerryHarvest.findAll(
                {
    ...
    async check(req, res) {
        ...
        let checkTable = await TableFruitBerryHarvest.findAll({where: {row_owner, date}});
        if (checkTable.length == 0) {
            var name = await TableFruitBerryHarvest.create({row_owner, date})
        }
    ...
    async create(req, res, next) {
        try {
            ...
            var name = await TableFruitBerryHarvest.update({
                row_owner: row_owner,
                value1: value1,
                value2: value2,
                value3: value3,
