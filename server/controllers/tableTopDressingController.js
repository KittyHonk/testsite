const {TableTopDressing} = require('../models/models')
...
class tableController {
    async getAll(req, res, next) {
        try {
            ...
            let table = await TableTopDressing.findAll(
                {
    ...
    async check(req, res) {
        ...
        let checkTable = await TableTopDressing.findAll({where: {row_owner, date}});
        if (checkTable.length == 0) {
            var name = await TableTopDressing.create({row_owner, date})
        }
    ...
    async create(req, res, next) {
        try {
            ...
            var name = await TableTopDressing.update({
                row_owner: row_owner,
                value1: value1,
                value2: value2,
                result12: result12,
